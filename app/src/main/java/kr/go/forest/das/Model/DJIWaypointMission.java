...
public class DJIWaypointMission {
    ...
    public DJIWaypointMission(List<GeoPoint> waypoints, GeoPoint base_point, float flight_speed, int interval){
        createDJIShootIntervalMission(waypoints, base_point, flight_speed);
    }
    ...
    private void createDJIShootIntervalMission(List<GeoPoint> waypoints, GeoPoint base_point, float flight_speed){
        waypoint_missions = new ArrayList<>();
        dji.common.mission.waypoint.WaypointMission.Builder builder = new dji.common.mission.waypoint.WaypointMission.Builder();
        ...
        for(int i = 0; i < group_count; i++) {
            for(int j = 0; j < WaypointMission.MAX_WAYPOINT_COUNT ; j++){
                ...
                GeoPoint _point = waypoints.get(_index);
                float _altitude = (float)base_point.getAltitude();
                Waypoint waypoint = new Waypoint(_point.getLatitude(), _point.getLongitude(), _altitude);
                LogWrapper.i("임무정보 인터벌2", String.format("lat : %f, lng : %f, alt : %f",_point.getLatitude(), _point.getLongitude(), _altitude));
                _waypoint_mission.add(waypoint);
    ...
    public  WaypointMission getDJIMission(GeoPoint base_point, Waypoint point, float flight_speed){
        dji.common.mission.waypoint.WaypointMission.Builder builder = new dji.common.mission.waypoint.WaypointMission.Builder();
        ...
        _waypoint_mission.add(new Waypoint(base_point.getLatitude(), base_point.getLongitude(), (float) base_point.getAltitude()));
        _waypoint_mission.add(point);

        builder.waypointList(_waypoint_mission).waypointCount(_waypoint_mission.size());
        WaypointMission waypoint_mission = builder.build();
        _waypoint_mission.clear();
